...
function escapePostbidConfig() {
  gulp
    .src("./integrationExamples/postbid/oas/postbid-config.js")
    .pipe(jsEscape())
  ...
  return gulp
    .src(
      [
        ...
        "!modules/trafficShapingModule.js",
        "./*.js",
      ],
      { base: "./" }
    )
    .pipe(
      eslint({
        fix: !argv.nolintfix,
        quiet: !(typeof argv.lintWarnings === "boolean"
          ? argv.lintWarnings
          : true),
      })
    )
...
function viewReview(done) {
  ...
  var reviewUrl =
    ...
    ":" +
    port +
    "/integrationExamples/reviewerTools/index.html"; // reuse the main port from 9999
...
function prebidSource(webpackCfg) {
  ...
  return gulp
    .src([].concat(moduleSources, analyticsSources, "src/prebid.js"))
...
function makeDevpackPkg(config = webpackConfig) {
  return function () {
    ...
    cloned.module.rules
      ...
      .forEach(
        (use) => (use.options = Object.assign({}, use.options, babelConfig))
      );
...
function makeWebpackPkg(config = webpackConfig) {
  ...
  return function buildBundle() {
    return prebidSource(cloned).pipe(gulp.dest("build/dist"));
  };
}
...
function buildCreative(mode = "production") {
  ...
  return function () {
    return gulp
      .src(["**/*"])
      .pipe(
        webpackStream(Object.assign(require("./webpack.creative.js"), opts))
      )
...
function updateCreativeExample(cb) {
  ...
  const root = require("node-html-parser").parse(
    fs.readFileSync(CREATIVE_EXAMPLE)
  );
  root.querySelectorAll("script")[0].textContent = fs.readFileSync(
    "build/creative/creative.js"
  );
...
function wrapWithHeaderAndFooter(dev, modules) {
  ...
  return function wrap(stream) {
    ...
    (dev
      ? Promise.resolve(tpl)
      : minify(tpl, { format: { comments: true } }).then((res) => res.code)
    )
      .then((tpl) => {
        // wrap source placeholder in an IIFE to make it an expression (so that it works with minify output)
        const parts = tpl
          .replace(placeholder, `(function(){$$${placeholder}$$})()`)
          .split(placeholder);
        if (parts.length !== 2) {
          throw new Error(
            `Cannot parse bundle template; it must contain exactly one instance of '${placeholder}'`
          );
        }
...
function bundle(dev, moduleArr) {
  ...
  if (modules.length === 0) {
    modules = allModules.filter(
      (module) => explicitModules.indexOf(module) === -1
    );
  } else {
  ...
  [coreFile]
    ...
    .forEach((file) => {
      (depGraph[file] || []).forEach((dep) =>
        dependencies.add(helpers.getBuiltPath(dev, dep))
      );
    });
  const entries = _.uniq(
    [coreFile].concat(Array.from(dependencies), moduleFiles)
  );
...
function testTaskMaker(options = {}) {
  ...
  options.disableFeatures =
    options.disableFeatures || helpers.getDisabledFeatures();
...
function runKarma(options, done) {
  ...
  options = Object.assign(
    { browsers: helpers.parseBrowserArgs(argv) },
    options
  );
...
function coveralls() {
  // 2nd arg is a dependency: 'test' must be finished
  // first send results of istanbul's test coverage to coveralls.io.
  return (
    gulp
      .src("gulpfile.js", { read: false }) // You have to give it a file, but you don't
      // have to read it.
      .pipe(
        shell(
...
function buildPostbid() {
  var fileContent = fs.readFileSync(
    "./build/postbid/postbid-config.js",
    "utf8"
  );

  return gulp
    .src("./integrationExamples/postbid/oas/postbid.js")
...
function startIntegServer(dev = false) {
  const args = [
    "./test/fake-server/index.js",
    `--port=${INTEG_SERVER_PORT}`,
...
const watch = watchTaskMaker({
  alsoWatch: ["test/**/*.js"],
  task: () => gulp.series(clean, gulp.parallel(lint, "build-bundle-dev", test)),
});
const watchFast = watchTaskMaker({
  livereload: false,
  task: () => gulp.series("build-bundle-dev"),
});
...
gulp.task(
  "build-creative-dev",
  gulp.series(
    buildCreative(argv.creativeDev ? "development" : "production"),
    updateCreativeRenderers
  )
);
gulp.task(
  "build-creative-prod",
  gulp.series(buildCreative(), updateCreativeRenderers)
);
...
gulp.task(
  ...
  gulp.series(
    "build-creative-dev",
    makeDevpackPkg(standaloneDebuggingConfig),
    makeDevpackPkg(),
    gulpBundle.bind(null, true)
  )
);
gulp.task(
  "build-bundle-prod",
  gulp.series(
    "build-creative-prod",
    makeWebpackPkg(standaloneDebuggingConfig),
    makeWebpackPkg(),
...
gulp.task(
  "build-bundle-verbose",
  gulp.series(
    "build-creative-dev",
    makeWebpackPkg(makeVerbose(standaloneDebuggingConfig)),
    makeWebpackPkg(makeVerbose()),
    gulpBundle.bind(null, true)
  )
);
...
gulp.task(
  ...
  testTaskMaker({
    disableFeatures: require("./features.json"),
    oneBrowser: "chrome",
    watch: false,
  })
);
gulp.task(
  "test",
  gulp.series(clean, lint, "test-all-features-disabled", "test-only")
);
...
gulp.task(
  "build",
  gulp.series(clean, "build-bundle-prod", updateCreativeExample)
);
gulp.task("build-postbid", gulp.series(escapePostbidConfig, buildPostbid));

gulp.task(
  "serve",
  gulp.series(clean, lint, gulp.parallel("build-bundle-dev", watch, test))
);
gulp.task(
  "serve-fast",
  gulp.series(clean, gulp.parallel("build-bundle-dev", watchFast))
);
gulp.task(
  "serve-prod",
  gulp.series(clean, gulp.parallel("build-bundle-prod", startLocalServer))
);
gulp.task(
  "serve-and-test",
  gulp.series(
    clean,
    gulp.parallel("build-bundle-dev", watchFast, testTaskMaker({ watch: true }))
  )
);
gulp.task(
  "serve-e2e",
  gulp.series(
    clean,
    "build-bundle-prod",
    gulp.parallel(() => startIntegServer(), startLocalServer)
  )
);
gulp.task(
  "serve-e2e-dev",
  gulp.series(
    clean,
    "build-bundle-dev",
    gulp.parallel(() => startIntegServer(true), startLocalServer)
  )
);
...
gulp.task(
  "e2e-test-only",
  gulp.series(requireNodeVersion(16), () => runWebdriver({ file: argv.file }))
);
gulp.task(
  "e2e-test",
  gulp.series(
    requireNodeVersion(16),
    clean,
    "build-bundle-prod",
    e2eTestTaskMaker()
  )
...
gulp.task(
  ...
  gulp.series(
    clean,
    lint,
    gulp.parallel("build-bundle-dev", watch, testCoverage),
    viewReview
  )
);
