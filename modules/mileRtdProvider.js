...
const storage = getStorageManager({ moduleType: MODULE_TYPE_RTD, moduleName: 'mile' });
...
const RTD_HOST = `https://rtd.mile.so`;
    ...
    if (rtdData) {
      ...
      ajax(url, {
        success: function (response, req) {
          ...
          if (req.status === 200) {
            try {
              ...
              reject(new Error(`Incorrect schema`, { cause: FETCH_FAILED_REASONS.INCORRECT_SCHEMA }));
            }
          } else {
            reject(new Error(`Non 200 status code`, { cause: FETCH_FAILED_REASONS.FETCH_FAILED }));
          }
        },
        error: function (_, options) {
          ...
          reject(new Error(`Fetch failed`, { cause: FETCH_FAILED_REASONS.FETCH_FAILED }));
        },
      });
    }
  });
...
function doNotShapeAuction(data, reason) {
  ...
  let fetched = reason !== DISABLED_REASONS.FETCH_FAILED;
