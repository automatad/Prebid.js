  ...
  while (queuePointer < __atmtdAnalyticsQueue.length) {
    ...
    try {
      switch (eventType) {
        ...
        case "slotRenderEnded":
          if (
            ...
            prettyLog(
              "error",
              `Could not process standalone ${eventType} without auctionInit from que as aggregator has not loaded. Will not be trying again ...`,
              true,
              () => {
      ...
      if (hasEventBeenProcessedSuccessfully) {
        prettyLog(
          "log",
          `Sucessfully processed event ${eventType} from que`,
          true,
          () => {
            console.log(`Event Raw Data :`, args);
          }
